...
export const lowerIsBetter = new Set(Object.keys(metricLabels));
...
export function improvement(oldValue, newValue, metric) {
...
export function getTargets(raw) {
  const targets = [];
  const seen = new Set();
...
export function getBaselineTarget(targets) {
...
export function getCompareTarget(targets, baselineTarget) {
...
export function createQualityBucket() {
  return {
    total: 0,
    valid: 0,
    invalid: 0,
...
export async function loadLatestRaw(directory = resultsDir) {
  const explicit = path.join(directory, 'latest-raw.json');
  try {
...
export async function loadRawResult(filePath) {
  return JSON.parse(await readFile(filePath, 'utf8'));
}

export async function listRawResults(directory = resultsDir) {
  const files = await readdir(directory).catch(() => []);
  return files
    .filter((file) => file.endsWith('-raw.json') || file === 'latest-raw.json')
    .sort()
    .reverse()
    .map((file) => path.join(directory, file));
}

export function buildComparison(raw, baselineId, compareId) {
  const targets = getTargets(raw);
  const baselineTarget = targets.find((target) => target.id === baselineId) || getBaselineTarget(targets);
  const compareTarget = targets.find((target) => target.id === compareId) || getCompareTarget(targets, baselineTarget);
  const { summary, quality } = summarize(raw);
  const metrics = {};

  for (const [metric, label] of Object.entries(metricLabels)) {
    const baseline = summary[baselineTarget?.id]?.[metric] || {};
    const compare = summary[compareTarget?.id]?.[metric] || {};
    metrics[metric] = {
      label,
      unit: metricUnits[metric],
      baselineMedian: baseline.median ?? null,
      compareMedian: compare.median ?? null,
      baselineP75: baseline.p75 ?? null,
      compareP75: compare.p75 ?? null,
      improvementPercent: improvement(baseline.median, compare.median, metric)
    };
  }
...
export async function writeMarkdownReport(raw, directory = resultsDir) {
  const targets = getTargets(raw);
  ...
  const markdownPath = path.join(directory, 'latest-report.md');
  const csvPath = path.join(directory, 'latest-summary.csv');
